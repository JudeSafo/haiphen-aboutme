// ---------------------------------------------------------------------------
// haiphen-risk — GCP Cloud Function entry point
//
// Wraps the CF Worker's route handler with Firestore-backed D1 adapter.
// Business logic (monte-carlo, var-engine, stress-test) is copied from
// haiphen-risk/src/ by the build script.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Route handler (matches haiphen-risk/src/index.ts)
// ---------------------------------------------------------------------------
...
async function route(req: Request, env: Env): Promise<Response> {
  ...
  if (req.method === "GET" && url.pathname === "/v1/risk/models") {
    return okJson({ models: MODELS_LIST }, requestId, cors);
  }
  ...
  if (req.method === "POST" && url.pathname === "/v1/risk/assess") {
    ...
    if (!body) return errJson("invalid_request", "Invalid JSON body", requestId, 400, cors);
    if (!body.scenario || typeof body.scenario !== "string" || body.scenario.length > 200) {
      return errJson("invalid_request", "Missing or invalid scenario (string, max 200 chars)", requestId, 400, cors);
    }
    if (!validatePortfolio(body.portfolio)) {
      return errJson("invalid_request", "Invalid portfolio: array of 1-50 assets with name, weight (0-1), expected_return, volatility (>0), current_value (>0)", requestId, 400, cors);
    }

    const model = body.model || "monte_carlo";
    if (!VALID_MODELS.includes(model as any)) {
      return errJson("invalid_request", `Invalid model. Must be one of: ${VALID_MODELS.join(", ")}`, requestId, 400, cors);
    }
    ...
    // ---- Persist to D1 (Firestore adapter) ----
    try {
